...
export class OperadoresComponent {
  ...
  fACL() {
    //Permissões: read = 9, new = 10, edit = 11, delete = 12
    //BLOQUEIA TUDO
    if (this.permissoes.includes(9, 10, 11, 12) == false) {
      this.read = false;
      this.new = false;
      this.edit = false;
      this.delete = false;
    ...
    //EDIT
    if (this.permissoes.includes(11)) {
      this.edit = true
    }
    //DELETE
    if (this.permissoes.includes(12)) {
      this.delete = true;
    }
  }
      ...
      if (form.value.email.indexOf('@') == -1) {
        ...
        if (this.validaEmail[0] == null || this.validaEmail[0] == '' || this.validaEmail[1] == null || this.validaEmail[1] == '') {
          ...
          if (form.value.telefone.indexOf('_') != -1) {
            ...
            this._HttpService.JSON_POST('/operadores', form.value, false, true)
              ...
              .then(
                res => {
                  ...
                  if (res.json().message === 'erro') return this._ToastrService.error('Já existe um Operador cadastrado com esses dados.', 'Erro!');
      ...
      if (form.value.email.indexOf('@') == -1) {
        ...
        if (this.validaEmail[0] == null || this.validaEmail[0] == '' || this.validaEmail[1] == null || this.validaEmail[1] == '') {
          ...
          if (form.value.telefone.indexOf('_') != -1) {
            ...
            this._HttpService.JSON_PUT(`/operadores/${this.GlobalService.getItem().id}`, form.value, false, true)
  ...
  *************/
  getItens() {
    ...
    this._HttpService.JSON_GET('/operadores', false, true)
  ...
  *************/
  deleteItem() {
    let res = confirm("Tem certeza que deseja excluir o operador?");

    if (res) {

      this._HttpService.JSON_DELETE(`/operadores/${this.GlobalService.getItem().id}`, false, true)
        .then(
          res => {
            this.getItens();
