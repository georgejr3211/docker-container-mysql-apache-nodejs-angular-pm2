exports.tg_update_alimentacao = async (migration, Sequelize) => {
    var user_tg_update_alimentacao = 'CREATE DEFINER=`fairtek`@`localhost` TRIGGER `fairtek`.`update_alimentacao` AFTER UPDATE ON `tb_animais_registros` FOR EACH ROW';
    await migration.sequelize.query(`
    ${user_tg_update_alimentacao}
    BEGIN
    ...
    END;
    `),
    console.log('::: TG UPDATE_ALIMENTACAO')
}