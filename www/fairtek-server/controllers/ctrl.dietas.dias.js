...
******************************************************************************************/
Router.route('/dias/list/:id_dieta')

//LISTA
.get(function(req, res, next){

    poolQuery.getConnection(function(err, connection) {
        connection.query(`SELECT * FROM tb_dieta_dias WHERE excluido <> 1 AND tb_dietas_id = ${req.params.id_dieta}`,
        ...
        connection.release();
            ...
            if (error){ res.status(400).send({ message: error.message }) }
                ...
                else { res.status(400).send({ message: `Não existem dias de dietas a serem exibidas!` }) }
            }
...
ROTA ITEM (ID)
******************************************************************************************/
Router.route('/:id/:id_dieta?')
  
//SELECIONA
.get(function(req , res, next){

    poolQuery.getConnection(function(err, connection) {
        connection.query(`SELECT * FROM tb_dieta_dias WHERE excluido <> 1 AND id = ${req.params.id} LIMIT 1`,
        function (error, rows, fields) {
        connection.release();
            ...
            if (error){ res.status(400).send({ message: error.message }) }
                ...
                if (rows.length > 0) { res.status(200).send(rows) }
                else { res.status(400).send({ message: `O dia da dieta pesquisado não existe!` }) }
            }
